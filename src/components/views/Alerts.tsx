...
export function Alerts() {
  ...
  const seedAlerts = async () => {
    ...
    try {
      for (const alert of ALERTS_SEED_DATA) {
        await setDoc(doc(db, `companies/${profile.companyId}/inventory_alerts`, `${profile.companyId}_${alert.id}`), {
          ...alert,
          id: `${profile.companyId}_${alert.id}`,
          createdAt: new Date().toISOString()
        });
      }
    } catch (error) {
      handleFirestoreError(error, OperationType.WRITE, 'inventory_alerts');
    }
  };
