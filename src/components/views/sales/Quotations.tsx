...
// Davis & Shirtliff themed Logo
const DsLogo = () => (
  <div className="border border-slate-900 p-2 font-sans max-w-[210px] bg-white inline-block">
    <div className="relative bg-[#0066cc] text-white p-2 font-black tracking-tighter text-sm flex flex-col items-start leading-none overflow-hidden rounded-sm">
      <div className="flex items-center gap-1">
        <span className="text-[10px] font-bold border border-white px-1 py-0.5 rounded-sm bg-[#0052a3]">D&S</span>
        <span className="text-xs uppercase tracking-wider font-extrabold">DAVIS &</span>
      </div>
      <span className="text-[15px] uppercase tracking-widest font-black mt-0.5">SHIRTLIFF</span>
      <div className="absolute right-0 bottom-0 top-0 w-12 opacity-15 flex flex-col justify-around pointer-events-none">
        <div className="h-0.5 bg-white rounded-full w-full transform rotate-12"></div>
        <div className="h-0.5 bg-white rounded-full w-full transform -rotate-12"></div>
        <div className="h-0.5 bg-white rounded-full w-full transform rotate-12"></div>
      </div>
    </div>
    <div className="text-[9px] text-slate-800 italic font-semibold mt-1 text-center font-serif">
      know <span className="font-sans font-bold text-[#0066cc]">H₂O</span> through experience
    </div>
  </div>
);

// High-fidelity Footer Icons
const FooterIcons = () => {
  const items = [
    { name: 'Water Pumps', icon: <Activity className="w-5 h-5 text-sky-600" /> },
    { name: 'Boreholes', icon: <Layers className="w-5 h-5 text-sky-600" /> },
    { name: 'Swimming Pool', icon: <Waves className="w-5 h-5 text-sky-600" /> },
    { name: 'Water Treatment', icon: <Droplets className="w-5 h-5 text-sky-600" /> },
    { name: 'Generators', icon: <Plug className="w-5 h-5 text-sky-600" /> },
    { name: 'Solar', icon: <Sun className="w-5 h-5 text-sky-600" /> },
    { name: 'Irrigation', icon: <Sprout className="w-5 h-5 text-sky-600" /> },
  ];
  return (
    <div className="flex justify-around items-center gap-2 py-3 border-t border-slate-200 mt-6 bg-slate-50/50 rounded-xl px-2">
      {items.map((item, idx) => (
        <div key={idx} className="flex flex-col items-center text-center">
          <div className="w-9 h-9 rounded-full border border-slate-200 flex items-center justify-center bg-white shadow-sm">
            {item.icon}
          </div>
...
export function Quotations() {
  ...
  useEffect(() => {
    if (isNewQuotationOpen && profile?.companyId) {
      ...
      if (company?.kraPin) {
        setSupplierKraPin(company.kraPin);
      }
      if (company?.address) {
  ...
  }, [isNewQuotationOpen, profile?.companyId, company, profile?.name]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-24 lg:pb-0">
      ...
      <AnimatePresence>
        {selectedQuotation && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center overflow-y-auto bg-slate-900/60 backdrop-blur-sm p-4 print:p-0 print:bg-white print:static print:overflow-visible">
            
            {/* Print Dialog Backdrop control button */}
            <div className="absolute top-4 right-4 flex items-center gap-3 z-50 print:hidden">
              <button
                onClick={handlePrint}
                className="flex items-center gap-2 bg-[#0066cc] hover:bg-blue-500 text-white px-5 py-2.5 rounded-xl font-bold shadow-lg text-sm transition-all"
              >
            ...
            <div className="bg-white w-full max-w-[820px] my-8 p-10 shadow-2xl rounded-sm border border-slate-200 font-sans text-slate-800 leading-relaxed text-left print:shadow-none print:border-none print:my-0 print:p-0 print:w-full select-text">
              ...
              <div className="border-[2px] border-double border-slate-900 p-6 min-h-[1050px] flex flex-col justify-between">
                ...
                <div>
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      ...
                      <div className="text-[10px] text-slate-700 mt-4 leading-normal font-sans">
                        <p className="font-bold text-slate-900">{selectedQuotation.supplierStore || 'Dundori Rd Main Store'}</p>
                        <p>PO Box 41762-00100</p>
                        <p>NAIROBI 100</p>
                        <p className="mt-2"><span className="font-bold">Tel:</span> {selectedQuotation.supplierPhone || '(+254 20) 6968 000, 558335'}</p>
                        <p><span className="font-bold">Email:</span> {selectedQuotation.supplierEmail || 'd&s@dayliff.com'}</p>
                        <p className="mt-1"><span className="font-bold text-slate-900">PIN No:</span> {selectedQuotation.supplierKraPin || 'P000591252N'}</p>
                        <p className="mt-3 font-semibold text-slate-900"><span className="font-bold">Salesperson:</span> {selectedQuotation.salesperson || 'Bashir Ouma'}</p>
                      </div>
                    </div>

                    {/* Right column quote metadata */}
                    <div className="text-right flex flex-col items-end">
                      <h1 className="text-2xl font-black text-slate-900 tracking-wider mb-3">SALES QUOTE</h1>
                      ...
                      <table className="text-xs text-left">
                        <tbody>
                          ...
                          <tr>
                            ...
                            <td className="font-bold text-[#0066cc]">{selectedQuotation.customerAccountNo || '157552'}</td>
                          </tr>
                        </tbody>
                      </table>
                  ...
                  {/* Items Table */}
                  <div className="mt-4 overflow-hidden border border-slate-900 rounded-sm">
                    <table className="w-full text-xs text-left border-collapse">
                      <thead>
                        <tr className="bg-[#0066cc] text-white font-black uppercase text-[10px] tracking-wider border-b border-slate-900 divide-x divide-slate-900">
                  ...
                  <div className="mt-6 text-[10px] space-y-1.5 border-t border-slate-200 pt-4 leading-normal font-sans text-slate-700">
                    <p><span className="font-black text-slate-950 uppercase w-20 inline-block">Delivery</span> {selectedQuotation.deliveryTerms || 'SUBJECT AVAILABILITY IN STOCK'}</p>
                    <p><span className="font-black text-slate-950 uppercase w-20 inline-block">Validity</span> {selectedQuotation.validityTerms || 'SUBJECT TO CONFIRMATION AT DATE OF ORDER'}</p>
                    <p><span className="font-black text-slate-950 uppercase w-20 inline-block">Payment</span> {selectedQuotation.paymentTerms || 'AS PER THE D&S WARRANTY TERMS'}</p>
                    <p><span className="font-black text-slate-950 uppercase w-20 inline-block">Warranty</span> {selectedQuotation.warrantyTerms || 'IN FULL WITH ORDER'}</p>
                  </div>
                ...
                <div>
                  ...
                  <div className="mt-6">
                    <div className="bg-[#0066cc] text-white px-3 py-1 text-[9px] font-black uppercase rounded-t-sm border border-slate-950">
                      Bank Details
                    </div>
                    <table className="w-full text-[9px] border-collapse border border-slate-950 font-semibold">
                      <thead>
                        <tr className="bg-slate-100 text-slate-500 border-b border-slate-950 divide-x divide-slate-950 text-left">
                      ...
                      <tbody>
                        <tr className="divide-x divide-slate-950 text-slate-900 bg-white">
                          <td className="px-2 py-1.5 font-bold">{selectedQuotation.bankAccountName || 'Davis & Shirtliff Ltd.'}</td>
                          <td className="px-2 py-1.5">{selectedQuotation.bankName || 'Standard Chartered'}</td>
                          <td className="px-2 py-1.5">{selectedQuotation.bankBranch || 'Chiromo'}</td>
                          <td className="px-2 py-1.5 font-bold">{selectedQuotation.bankCurrency || 'KES'}</td>
                          <td className="px-2 py-1.5 font-black text-slate-950">{selectedQuotation.bankAccountNo || '0104033637700'}</td>
                          <td className="px-2 py-1.5">{selectedQuotation.bankCode || '02'}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                  ...
                  <div className="mt-4 border border-slate-900 p-3 bg-slate-50/50 rounded-sm">
                    <p className="text-[#0066cc] font-black uppercase text-[9px] tracking-wider mb-3">Quote Confirmation</p>
                  ...
                  <div className="text-center text-xs font-black tracking-wider text-slate-500 uppercase mt-4">
                    Water & Energy <span className="text-[#00ccff]">Solutions</span> for Africa
                  </div>
                </div>
